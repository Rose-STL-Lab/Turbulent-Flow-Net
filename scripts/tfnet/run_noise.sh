...
for data in data20_101; do
    for i in "${!array[@]}"; do
        ...
        name=north_tfnet_${data}${dnsn}_noise_${noise}
        folder=${name}/${name}_${seed}
        mkdir -p results/$folder
        cp -v ${BASH_SOURCE[0]} results/$folder/
        python TF_net/run_model.py  --version $version --desc $name ${dnsn} --noise ${noise} --data ${data}.pt --coef 0 --coef2 $coef2 --seed ${seed} --d_ids $d_id --path results/$folder/ \
