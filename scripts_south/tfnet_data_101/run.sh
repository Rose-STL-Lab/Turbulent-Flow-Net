...
for beta in 0.7 0.6 0.5; do
    for i in "${!array[@]}"; do
        ...
        d_id="${array2[i]}"
        echo $seed $d_id
        name=${server}_tfnet_${data}_outln_${outln}_beta_${beta}
        folder=${name}/${name}_${seed}
        mkdir -p results/$folder
        cp -v ${BASH_SOURCE[0]} results/$folder/
        python TF_net/run_model.py  --output_length ${outln} --beta ${beta} --desc $name --data ${data}.pt --coef 0 --coef2 $coef2 --seed ${seed} --d_ids $d_id --path results/$folder/ \
